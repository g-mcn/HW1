Shader "Custom/Edge"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "Thea_AdditionalArt_01" {}
        _Mix ("Mix", Float) = 0
        _LookUpDistance ("LookUpDistance", Int) = 1
		_Cells ("Cells", Float) = 2
    }
    SubShader
    {
        ...
        float3x3 Gx = float3x3( 0, -2, 0, 0, 0, 0, 1, 2, 1 ); // x direction kernel
        float3x3 Gy = float3x3( -1, 0, 0, -2, 0, 2, 0, -1, 1 ); // y direction kernel

        
        // fetch the 3x3 neighborhood of a fragment
        float tx0y0 = tex2D( _MainTex, i.uv + texel * float2( 0, 0 ) ).r;
       ...
       // else if (_Mix < 0.0) { _Mix = 0.0; }
        ...
        //stripes
        float sum =+ floor(i.uv.xy * _Cells);  // + floor(i.uv.xy * numCells) floor(i.uv.x * _Cells)
        //checkerboard
